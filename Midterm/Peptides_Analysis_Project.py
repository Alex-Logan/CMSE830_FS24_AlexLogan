...
# Closing Notes page
    ...
    The main challenge was finding a meaningful way to **incorporate the datasets together**- outside the type of cancer, there wasn't a meaningful column to merge on. Because of that, assumptions made about the peptides' potential effects on the causes of breast cancer from a completely different dataset have to be taken as **hypothetical**- though there **is** some reason to believe they are worth considering- namely that we are looking **at the same type of cancer in both datasets** - these are anticancer peptides for **breast cancer** so if they work, it's not entirely unreasonable to suggest certain potential conclusions based on what we know about breast cancer. I'd have liked an even stronger leg to stand on, but making data science projects out of merging separate datasets together isn't easy! You'll notice that **aside from some visualizations at the end**, the merged dataset never really got used- there never really ended up being a **meaningful reason to have it** and it was easier to just keep things separate for analysis. **I intentionally made a point to emphasize this in the project by turning the notion of using the merged dataset into something of a running joke.** This project taught me many valuable lessons on how far we can expect traditional data science to take us on its own- and how challenging it is to do truly meaningful research! No wonder we've all heard that sobering statistic that the large majority of data science projects allegedly fail.
    ...
    However, it's less clear whether or not we were onto quite as much with the other specific amino acids. You might remember that earlier in the project I refused to use SMOTE because I was worried about too much fake data. But those **same class imbalances** made things much harder to analyze in the end when it came to the peptides, especially ones that weren't Lysine. Lesson learned for the future- SMOTE has its place! Ultimately, I think I should've stuck with it, but it was perhaps even more valuable to learn **why** I should have.
